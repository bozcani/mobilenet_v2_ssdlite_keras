...
def decode_detections_fast(y_pred,
    ...
    if input_coords == 'centroids':
        ...
        # delta_xmin(pred) / w(anchor) * w(anchor) == delta_xmin(pred),
        # delta_xmax(pred) / w(anchor) * w(anchor) == delta_xmax(pred)
        y_pred_converted[:, :, [2, 4]] *= np.expand_dims(y_pred[:, :, -6] - y_pred[:, :, -8], axis=-1)
        # delta_ymin(pred) / h(anchor) * h(anchor) == delta_ymin(pred),
        # delta_ymax(pred) / h(anchor) * h(anchor) == delta_ymax(pred)
        y_pred_converted[:, :, [3, 5]] *= np.expand_dims(y_pred[:, :, -5] - y_pred[:, :, -7], axis=-1)
        y_pred_converted[:, :, 2:] += y_pred[:, :, -8:-4]  # delta(pred) + anchor == pred for all four coordinates
    else:
...
def decode_detections_debug(y_pred,
    ...
    for batch_item in y_pred_decoded_raw:  # `batch_item` has shape `[n_boxes, n_classes + 4 coords]`
        ...
        for class_id in range(1, n_classes):  # For each class except the background class (which has class ID 0)...
            ...
            # ...keep only those boxes with a confidence above the set threshold.
            threshold_met = single_class[single_class[:, 1] > confidence_thresh]
            if threshold_met.shape[0] > 0:  # If any boxes made the threshold...
    ...
    for batch_item in y_pred_decoded:
        ...
        for prediction in batch_item:
            if (prediction[0] < 0) or (prediction[0] >= cum_boxes_per_pred_layer[-1]):
                raise ValueError(
                    "Box index is out of bounds of the possible indices as \
                    given by the values in `num_boxes_per_pred_layer`.")
